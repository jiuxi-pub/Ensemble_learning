...
cwd = os.getcwd()
print (cwd)
data_path = cwd+'/Datasets/gisette/gisette_train.csv'
df = pd.read_csv(data_path, sep=' ', header=None)
my_data = df.iloc[:, :-1]
print (my_data.shape)
label_path = cwd+'/Datasets/gisette/gisette_train_labels.csv'
# label_path = '/home/jiuxi/playground/Ensemble_Learning/Datasets/gisette/gisette_train_labels.csv'
...
# %%
# from_records convert the array to pandas dataframe
# here we use it to reconstruct the index
...
_, fat_acc = evaluate_model_mlp(X_train_scaled, y_train, X_test_scaled, y_test, input_dim = 5000, verbose=0, shuffle=True)
...
density = 'auto'
num_splits = 15
X_train_thin_sets, X_valid_thin_sets, X_test_thin_sets, y_train_thin_sets, y_valid_thin_sets = rp_data_gen_reverse(X_train_fixed, y_train, X_test_fixed, n_splits=num_splits, thin_dim=thin_dim, density=density)
...
cwd = os.getcwd()
file_name = cwd+'/results/gisette_mlp_plotdataY.csv'
df.to_csv(file_name)
